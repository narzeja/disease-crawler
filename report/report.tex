...
\textit{synonyms}, we utilize the \textit{Google Search Engine} to
harvest additional knowledge to expand the model. [The models are
constructed using machine learning and natural language processing
techniques by text mining symptoms and features from abstracts and other
relevant paragraphs of text found in the search process.]
...
\section{Method}
\label{chap:method}
\todo[inline]{Give a proper introduction to chapter}
...
save it for further processing given some acceptance threshold.
...
constructed in \cite{jensenandersen}. While \cite{jensenandersen} used
information gathered by automatically searching the Pubmed online
database \footnote{\url{http://www.ncbi.nlm.nih.gov/pubmed/}}, prone to
contain irrelevant information, we use validated abstracts provided by
Orphanet.
...
non-alphanumerical \footnote{Symbols not represented in the Danish
Alphabet} symbols are removed .
...
%Chapter \ref{chap:implementation} explains how this technique are
...
ICD10\footnote{"International Statistical Classification of Diseases and Related Health Problems"-10th revision.} codes from Orphanet represents a hirachical structured coding of the category of the disease. As an example, Cholera falls within \textit{A00 $\rightarrow$ 'Certain infectious and parasitic diseases' $\rightarrow$ 'Intestinal infectious diseases' $\rightarrow$ 'Cholera' }. Codes can thus be used to locate groups or clusters of diseases and has the potential being used for training models to be used for classifying un-coded diseases. In theory limiting a diagnostic query to one or a few categories should improve the precision of the current model and --- using the TFIDF-model --- important terms of each category can be located (main symptoms). If these main terms are found in the symptomatic query of the physician, potential categories can be found. Having limited the query to a subspace, the model can be fine-tuned to locate subtle features or symptoms in the symptomatic query of the reduced set of diseases. Intuitively, reusing the TFIDF scheme on the recued space should, reduce the importance of main features or symptoms and enhance the more subtle parts. 

BLABLABLA

 Though 3335 ICD10 codes were received from Orphanet (in total representing 3137 unique rare diseases) only 50.05\% of these were also represented in the used data set (diseases containing Orphanet abstracts). 


\todo[inline]{Explain this method!}
...
particular, a basic \textit{unigram} model - noise reduced, an
\textit{NLP+unigram} model utilizing a consensus ranking scheme across
two TF-IDF matrices and finally an \textit{ICD10 code} based model is
constructed and ultimately used for testing and comparison.
...
ranks tied for, with no correction for ties.
...
\begin{tabular}{lll}
    ...
    $H_{crit}$ & & 3.84 \\
\end{tabular}
    \captionof{table}{Results from the Kruskal Wallis
    significance test of the Orphanet tests on the old and new model.}
