...
the candidates that appear more than once or a supterm of other
candidates. I.e., consider the candidates $\mathbb{S} = $ \texttt{\{bleeding diathesis,
bleeding, sleep apnae\}} we will return the symptoms $\mathbb{S'} = $ \texttt{\{bleeding
diathesis, bleeding\}}.

$ s_{j}, s_{i} \in \mathbb{S'}$ if count$(s_{j}) > 1,$ count$(s_{i}) \wedge s_{j} \subset s_{i}, $for $s_{i}, s_{j} \in \mathbb{S} $

Anecdotal observations during the development phase of this module
showed that returning terms and supterms of those terms increased the
relevance of the returned symptoms.
...
tagging\fxwarning{Brief explanation here}, and semantic
